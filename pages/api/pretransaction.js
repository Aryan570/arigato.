...
const handler = async (req, res) => {
    if (req.method == 'POST') {
        ...
        for (let item in cart) {
            ...
            //check if cart items are out of stocks ---pending
            if (product.availableQty < cart[item].qty) {
                res.status(200).json({ success: false, "error": "Some items in your cart went out of stock. Please try again, later.", cartClear: true })
            }
            if (product.price !== cart[item].price) {
                res.status(200).json({ success: false, "error": "The price of sum items in your cart has been changed, Please try again", cartClear: true })
                return
            }
