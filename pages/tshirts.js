          ...
          {Object.keys(products).length===0 && <p>Sorry! ALl the Stickers are currently out of stock. New Stock coming soon. Stay tuned</p>}
            return <Link passHref={true} key={products[item]._id} href={`/product/${products[item].slug}`}><div className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer shadow-lg m-5">
              ...
              <div className="mt-4 text-center md:text-left">
                <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Jerseys</h3>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'jersey' })
  let jerseys = {}
  for (let item of products) {
    if (item.title in jerseys) {
      ...
      if (item.availableQty > 0) {
        ...
        jerseys[item.title].color = []
        jerseys[item.title].size = []
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(jerseys)) }, // will be passed to the page component as props
  }
}
